...
# Formats the user's category/transaction data into a map
def get_category_data():
    ...
    for category_row in category_rows:
        ...
        transactions_rows = cursor.execute("SELECT * FROM TRANSACTIONS WHERE User_Name=? AND Category_Name=?",
                                            (session['username'],category_row[0],)).fetchall()
...
# Endpoint for displaying the landing page
@app.route('/')
def index():
    return render_template('index.html', title='Landing Page')

# Endpoint for a user to signup
...
def login():
    if request.method == 'POST':
        ...
        row = cursor.execute("SELECT User_Name, Password FROM USER WHERE User_Name=? AND Password=?",
...
# Endpoint for the user's homepage
@app.route('/home')
def home():
...
def createCategory():
    ...
    row = cursor.execute("SELECT * FROM CATEGORY WHERE Category_Name=? AND User_Name=?",
                            (category_name, session['username'],)).fetchone()
    ...
    if row is None and is_acceptable_number(budget):
        ...
        print("Could not create category. Category already exists and/or budget input is invalid.")
        flash("Could not create category. Category already exists and/or budget input is invalid.")
...
def updateCategory():
    ...
    row = cursor.execute("SELECT * FROM CATEGORY WHERE Category_Name=? AND User_Name=?",
                            (old_category_name, session['username'],)).fetchone()

    new_category_name_exists = cursor.execute("SELECT * FROM CATEGORY WHERE Category_Name=? AND User_Name=?",
                                                (new_category_name, session['username'],)).fetchone()
    ...
    # The row should exist if we want to update it
    if row is not None and new_category_name_exists is None and is_acceptable_number(new_budget):
        # Update the CATEGORY table
        cursor.execute("UPDATE CATEGORY SET Category_Name=?, Budget_Amount=? WHERE Category_Name=? AND User_Name=?",
                        (new_category_name, f'{float(new_budget):.2f}', old_category_name, session['username'],))

        # Update the TRANSACTIONS table
        cursor.execute("UPDATE TRANSACTIONS SET Category_Name=? WHERE Category_Name=? AND User_Name=?",
                        (new_category_name, old_category_name, session['username'],))

        connection.commit()
    else:
        print("Could not update category. Old category either does not exist, new category already exists and/or new budget input is invalid.")
...
def deleteCategory():
    ...
    if row is not None:
        ...
        print("Could not delete category. Category does not exist.")
...
def addTransaction():
    ...
    if row is not None and is_acceptable_number(money_spent):
        cursor.execute("INSERT INTO TRANSACTIONS VALUES (?, ?, ?, ?, ?, ?)",
                        (None, category_name, session['username'], transaction_description, f'{float(money_spent):.2f}', date))

        count = 0
        count = cursor.execute("SELECT COUNT(*) FROM TRANSACTIONS WHERE Category_Name=? AND User_Name=?",
                                (category_name, session['username'],)).fetchone()

        if count:
            cursor.execute("UPDATE TRANSACTIONS SET Transaction_ID=? WHERE Category_Name=? AND User_Name=? AND Date=?",
                            (count[0], category_name, session['username'], date))
        
        connection.commit()
    else:
        print("Could not add transaction. Category does not exist and/or money spent input is invalid.")
...
def updateTransaction():
    ...
    row = cursor.execute("SELECT * FROM TRANSACTIONS WHERE Transaction_ID=? AND Category_Name=? AND User_Name=?",
                            (old_transaction_id, category_name, session['username'],)).fetchone()

    # The transaction must exist in order to update it
    if row and is_acceptable_number(new_money_spent):
        cursor.execute("UPDATE TRANSACTIONS SET Description=?, Money_Spent=? WHERE Transaction_ID=? AND Category_Name=? AND User_Name=?",
                        (new_transaction_description, f'{float(new_money_spent):.2f}', old_transaction_id, category_name, session['username']))

        connection.commit()
    
    else:
        print("Could not update transaction. Category does not exist, transaction id is invalid and/or new money spent input is invalid.")
...
def deleteTransaction():
    ...
    row = cursor.execute("SELECT * FROM TRANSACTIONS WHERE Transaction_ID=? AND Category_Name=? AND User_Name=?",
                            (transaction_id, category_name, session['username'],)).fetchone()

    # The transaction must exist in order to delete it
    if row:
        cursor.execute("DELETE FROM TRANSACTIONS WHERE Transaction_ID=? AND Category_Name=? AND User_Name=?",
                        (transaction_id, category_name, session['username']))

        transactions = cursor.execute("SELECT * FROM TRANSACTIONS WHERE Category_Name=? AND User_Name=? ORDER BY Date",
                                        (category_name, session['username'],)).fetchall()

        id_counter = 1
        for transaction in transactions:
            cursor.execute("UPDATE TRANSACTIONS SET Transaction_ID=? WHERE Transaction_ID=? AND Category_Name=? AND User_Name=?",
                            (id_counter, transaction[0], category_name, session['username']))
            id_counter += 1

        connection.commit()
    else:
        print("Could not delete transaction. Category does not exist and/or transaction id is invalid.")
